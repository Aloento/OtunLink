...
describe('buildMsalConfig', () => {
  const input = {
    tenantId: 'tenant-1',
    clientId: 'client-1',
    redirectUri: 'https://otun.musi.land/auth/callback',
    postLogoutRedirectUri: 'https://otun.musi.land',
  ...
  it('使用 AAD 权威与重定向地址', () => {
    ...
    expect(msal.auth.redirectUri).toBe('https://otun.musi.land/auth/callback');
  });
