...
| API Token       | 存于 `<repo>\.cf\api-token.txt`（gitignored；权限：Worker/Pages/KV/Hyperdrive 编辑）                                                                                                                                                         |
| API Worker      | `otunlink-api` 已部署（2026-08-31）；自定义域 `api.otun.musi.land`（自动创建 CNAME+证书）与 workers.dev 子域 `otunlink-api.<account>.workers.dev` 均已启用                                                                                              |
...
| 项目           | 值                                                                                                                                                                              |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 主机 / 端口    | `<DB_HOST>:5432`（公司私有，需 Hyperdrive 隧道）                                                                                                                                  |
...
说明：用户回传的 `<S3_BUCKET>.<OBS_ENDPOINT>` 是「bucket 前缀」访问地址，
...
| API 暴露范围（scope） | `https://<TENANT_DOMAIN>.onmicrosoft.com/OtunLink/API`（发布者域形式；token `aud` 即此 URI）                                                                                |
...
$env:CLOUDFLARE_API_TOKEN = (Get-Content '<repo>\.cf\api-token.txt' -Raw).Trim()
