...
| R01–R06 | ✅（2026-09-01 已复验） | 已按 §8.1 语义复验（零售=外部合作方）：① 库存/零售价仅显示**已签约仓库**（奥地利仓，无匈牙利仓），零售价只读无添加/编辑（仅「历史」）；② 请货单可创建（选签约仓库+物品+数量→DRAFT，服务端校验零售价/库存快照），收货门店锁定自身；③ 销售单详情可见仓库填写的**配送方式+运单号**（DHL / TRK-20260901-0001），无发货单单号；④ 导航无发货单入口，直达 `/shipments`、`/items/new`、`/items/:id/edit` 均被「无权访问」拦截；⑤ 物品目录无「新建物品」、详情无「编辑」按钮 |
...
**实现完成（2026-09-01）**：角色权限 + scope + 签约 + 销售单配送字段已全部实现（前序修复任务），全量 `pnpm -r test` / `pnpm -r build` 通过；R01–R06 待按 §8.1 预期复验。

**UI 复验完成（2026-09-01，零售链路 R）**：以上 R 链路已用 `retail.tester` 实测通过（详见本文件 §4 状态表 R01–R06 行与下述复验记录）；复验中还额外修复 3 处前端权限/过滤遗漏：物品目录/详情按钮门控（ITEMS_WRITE）、零售价页仓库下拉按签约加载、请货页仓库下拉按签约加载 + 仓库/零售买卖双方下拉 disabled 逻辑（误锁空值）。复验数据：本地库补充 WH-AT↔ST-XX 零售价 15.00 EUR、库存 100（批次效期延至 2026-12-31）与 `sales_orders.carrier/tracking_no`（0008 迁移已应用），以便走通请货→发送→零售查看配送链路。
