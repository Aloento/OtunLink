...
- 权限：WAREHOUSE（创建/发送/取消/改零售价）、RETAILER（请货/查看库存与零售价/上传支付/确认收货）；scope 按买方或卖方单元过滤（SALES_REQUEST/SALES_CREATE/SALES_SEND/SALES_CANCEL/SALES_PAYMENT/SALES_CONFIRM_RECEIPT）。 **✅ 2026-09-01 已按评审语义重实现**：本 checkpoint 的销售单/配送语义已按 `docs/qa/rbac-matrix-vs-semantics.md` §1.1 评审结论重实现（ck-09a 销售单/配送：零售价只读且范围=已签约仓库；请货可选仓库=已签约；销售单含配送方式+配送单号，仓库填写、零售只读；scope 非 ADMIN 必填），设计文档以 design.md v1.4 为准。
...
- 遗留：内存仓库 stock qty 以字符串输出（'4'）与 SQL 数值类型差异；售后退货属 ck-09b。
- 注：早期「RETAILER 无 scope 时只能看自己单元库存」已过时——2026-09-01 起非 ADMIN 空 scope 一律 403，零售库存按已签约仓库过滤（见 rbac-matrix-vs-semantics.md §6）。
