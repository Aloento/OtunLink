...
> 版本：v1.3（2026-08-31 晚：确认角色权限语义 + 新增仓库-零售签约关系、销售单配送信息） · 状态：待实现 · 定位：公司内部（集货/仓库/管理）+ 外部合作方（零售门店/商铺买家）协同系统（多集货部 → 多欧洲仓 → 多零售门店）
...
| `users.scope_unit_id` | 归属单元，**非 ADMIN 必填**（2026-08-31 确认：**一个账户只能绑定一个实体**，即 1 账号 = 1 仓库/集货地/门店，不允许一个账号管理多个；**一个实体可绑定多个账户**，如一个仓库可有多个仓管账号——`scope_unit_id` 单值 + 多 users 行即天然支持）；ADMIN 可空（全量）。如某仓管归属匈牙利仓、某店长归属 XX 超市。⚠️ 当前实现「空 = 本岗位全量」需修复（见本节约注与 §11.10） |
...
> 修订记录（2026-08-31）：① RETAILER 原矩阵「发货单只读」「物流单号 ✓」「查看零售价只读」——已按业务语义改为「发货单不可见」「物流单号不可见」「零售价只读（范围=已签约仓库）」；② 明确 RETAILER 为**外部合作方（商铺买家）**；③ WAREHOUSE/COLLECTOR/RETAILER 必须绑定归属单元（**一个账户只能绑定一个实体**；一个实体可有多个账户；仅 ADMIN 例外）；④ 新增「仓库-零售签约关系」（见 §3.2.1）与「销售单配送信息」（见 §4.2 销售单）；⑤ 待实现项见 §11.10。
...
- 数据范围（2026-08-31 确认）：**非 ADMIN 的 `scope_unit_id` 必填**（一个账户只能绑定一个实体；一个实体可有多个账户）；仅 ADMIN 可空（全量）。所有查询/写操作强制附加归属条件（组合成 SQL `where AND`，避免泄漏）；不允许「空 = 全量」的放行逻辑（`scopeAllows` 系列需改为空 scope 即拒绝，ADMIN 例外）
  - **零售（RETAILER）例外**：库存/零售价的查询范围 = **已签约仓库集合**（见 §3.2.1），**不**附加自身门店 `scope_unit_id` 过滤——当前 `GET /stock`、`GET /retail-prices` 以 `unitId || scope.unitId` 兜底会把零售限制到自己的门店（与「零售看各仓库库存」冲突），需修复
...
#### 仓库-零售签约 `retail_partnerships`（2026-08-31 新增，待实现）
...
10. **角色权限模型偏差（2026-08-31 发现，2026-08-31 晚语义已确认 → 待实现）**：已实现能力与业务语义不符——RETAILER 应**不可见发货单**、**不可管理物品**、零售价**仅只读**（范围=已签约仓库）；仓库/集货/零售**必须绑定归属单元**（一个账户只能绑定一个实体，一个实体可有多个账户；仅 ADMIN 例外）；需新增**仓库-零售签约关系**（§3.2.1）与**销售单配送方式+单号**（§3.2.2）。详见 `docs/qa/rbac-matrix-vs-semantics.md`（§1.1 确认结果）与 `docs/qa/ui-acceptance.md` §8.1。
