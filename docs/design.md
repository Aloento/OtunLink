...
> 版本：v1.4（2026-09-01 实现） · 状态：已实现（2026-09-01） · 定位：公司内部（集货/仓库/管理）+ 外部合作方（零售门店/商铺买家）协同系统（多集货部 → 多欧洲仓 → 多零售门店）
...
一套**公司内部（集货/仓库/管理）+ 外部合作方（零售门店/商铺买家）共用的私域 B2B 供应链协同 ERP**：让"集货方 → 仓库方 → 零售方"之间的单据流转、库存变动（按批次/效期）、价格管理全部线上化、可审计、双语（中/英）、多终端（桌面/手机/平板）可用。
仓库与零售之间的环节按**标准电商闭环**设计（下单 → 支付 → 发货/自提 → 收货 → 售后退货）；物流清关由专业货代外包，**系统不涉及报关/清关/单证**。

### 1.3 使用范围（2026-08-31 按业务语义修订）

- **内部岗位**：集货部、仓库、管理员为公司员工，用公司自有 Azure AD（Entra ID）免费版账号登录（单租户 App Registration）
- **外部合作方**：**零售岗位（RETAILER）是给其他外部合作方的（商铺买家），不是公司员工**。账号由**公司创建 Entra 账号**（与内部其他用户一致的流程：管理员在「用户管理」分配 RETAILER 岗位、绑定其门店单位），无特殊 B2B 来宾流程
- **角色是岗位、对象是多个**：`集货部×N`（上海、广州…）、`仓库×N`（匈牙利仓、奥地利仓…）、`零售门店×N`（XX 超市、YY 超市…），业务单据必须落到具体业务单元
- **数据范围（归属）**：仓库角色**只能管理自己归属的仓库**；集货角色**只能管理自己归属/经手的发货单**；零售角色绑定所属门店（买方），**只能查看已签约仓库的库存后请货**（签约由仓库主动添加，见 §3.2.1）
...
| 集货方 | Collector | 中国集货部门（上海/广州…），发货单创建者与发货方；**只能管理自己归属/经手的发货单** |
| 仓库方 | Warehouse | 欧洲仓库（匈牙利/奥地利…），收货点货、库存、出入库、销售；**负责设置/修改本仓库零售价**（零售价是仓库维度）；**只能管理自己归属的仓库** |
| 零售方 | Retailer | **外部合作方（商铺买家）**：零售门店（XX/YY 超市…），请货（下单）、付款、收货、售后；**不可见发货单、不管理零售价**，只能查看各仓库库存后请货 |
...
- B2C/散户、**其他**外部公司接入（注意：零售门店本身就是外部合作方，见 §1.3，不属于此列）、多租户 SaaS（预留：`customers` 个体客户实体 + 销售单买家多态，见 8.10）
- 报关/清关/单证（外包货代负责）
- 库位管理、库存盘点单、序列号
...
用户 = 岗位（决定能力）+ 归属单元（决定可管理对象）。**2026-08-31 按业务语义确认**：仓库只能管自己归属的仓库、集货只能管自己的发货单、零售为外部合作方（不可见发货单/不管理零售价）。**仓库/集货/零售必须绑定归属单元**：**一个账户只能绑定一个实体**（1 账号 = 1 仓库/集货地/门店，不允许一个账号管理多个）；**一个实体可绑定多个账户**（如一个仓库可有多个仓管账号）；仅 ADMIN 可空（全量）。

| 字段 | 说明 |
|---|---|
| `users.role` | `ADMIN` 管理员 / `COLLECTOR` 集货 / `WAREHOUSE` 仓库 / `RETAILER` 零售（外部合作方） |
| `users.scope_unit_id` | 归属单元，**非 ADMIN 必填**（2026-08-31 确认：**一个账户只能绑定一个实体**，即 1 账号 = 1 仓库/集货地/门店，不允许一个账号管理多个；**一个实体可绑定多个账户**，如一个仓库可有多个仓管账号——`scope_unit_id` 单值 + 多 users 行即天然支持）；ADMIN 可空（全量）。如某仓管归属匈牙利仓、某店长归属 XX 超市。✅ 已实现（2026-09-01）：非 ADMIN 强制绑定归属单元，「空 = 全量」仅 ADMIN 有效（见本节约注与 §11.10） |

| 能力 | COLLECTOR | WAREHOUSE | RETAILER | ADMIN |
|---|---|---|---|---|
| 物品搜索/浏览/扫码 | ✓ | ✓ | ✓ | ✓ |
| 物品新增/编辑 | ✓ | ✓ | ✗ **不可管理**（仅浏览/搜索/扫码） | ✓ |
| 创建/编辑/转交发货单（可选具体集货部） | ✓ | 只读 | ✗ **不可见** | ✓ |
| 绑定/查看多物流单号（发货单） | ✓ | ✓ | ✗ **不可见**（随发货单；零售看销售单配送信息） | ✓ |
| 点货（填实收） | 只读 | ✓ | ✗ | ✓ |
| 提交差异修订 | ✗ | ✓ | ✗ | ✓ |
| 审批/拒绝差异修订 | ✓ | ✗ | ✗ | ✓ |
| 确认收货 / 入库建档（批次/效期） | ✗ | ✓ | ✗ | ✓ |
...
| 手动入库/出库/报损（含过期报损） | ✗ | ✓ | ✗ | ✓ |
| 修改零售价 | ✗ | ✓（仅本仓库） | ✗ **不可管理** | ✓ |
| 查看库存/批次效期（只读） | ✗ | ✓ | ✓（**已签约仓库**，非仅自己门店） | ✓ |
| 查看零售价（只读） | ✗ | ✓ | ✓（**只读**仓库提供的零售价；范围 = 已签约仓库） | ✓ |
| 请货（创建请货草稿）/上传支付凭证/确认收货 | ✗ | ✗ | ✓ | ✓ |
| 发送/取消销售单 | ✗ | ✓ | ✗（只能请货） | ✓ |
| **发起售后退货** | ✗ | ✗ | ✓ | ✓ |
| **审核/收货退回（回补库存）** | ✗ | ✓ | ✗ | ✓ |
| 用户/部门/审计管理 | ✗ | ✗ | ✗ | ✓ |

> 修订记录（2026-08-31）：① RETAILER 原矩阵「发货单只读」「物流单号 ✓」「查看零售价只读」——已按业务语义改为「发货单不可见」「物流单号不可见」「零售价只读（范围=已签约仓库）」；② 明确 RETAILER 为**外部合作方（商铺买家）**；③ WAREHOUSE/COLLECTOR/RETAILER 必须绑定归属单元（**一个账户只能绑定一个实体**；一个实体可有多个账户；仅 ADMIN 例外）；④ 新增「仓库-零售签约关系」（见 §3.2.1）与「销售单配送信息」（见 §4.2 销售单）；⑤ 已实现（2026-09-01），详见 §11.10。

### 3.2.1 仓库-零售签约（供货）关系（2026-08-31 新增需求）

- 零售可见的**仓库库存、零售价**，以及请货时**可选的仓库** = **已签约仓库集合**（默认空 → 零售看不到任何仓库数据）。
- **签约只能由仓库主动发起**：WAREHOUSE 在其归属仓库下把零售门店「添加进可售客户列表」即生效；**零售方无需同意**（无确认状态机）。
- 仓库可删除客户（解约）；解约后零售立即不可见该仓库库存/零售价、不可再对该仓库请货（存量销售单不受影响）。
- 数据模型（见 §4.2 `retail_partnerships`）：`warehouse_unit_id + retailer_unit_id` 唯一；`created_by/created_at` 审计。
- 访问控制：WAREHOUSE 仅能维护**自己归属仓库**的客户列表；ADMIN 可管理全部。
- 影响面：零售库存查询、零售价查询、请货可选仓库、销售单创建校验（`seller_unit_id ∈ 已签约仓库`）。

### 3.2.2 销售单配送信息（2026-08-31 新增需求）

- 零售看不到发货单/发货单物流单号；但**销售单**上由仓库（卖方）提供**配送方式 + 配送单号**（如自提、快递、物流）供零售查看。
- 字段：`sales_orders.delivery_method`（已有 `PICKUP/EXPRESS/LOGISTICS`）+ **新增** `carrier`（配送商，可空）+ `tracking_no`（配送单号，可空）。
- 填写时机：仓库发送/发货销售单时填写；零售在销售单详情**只读**查看（独立于发货单 `shipment_trackings`）。
...
- 数据范围（2026-08-31 确认）：**非 ADMIN 的 `scope_unit_id` 必填**（一个账户只能绑定一个实体；一个实体可有多个账户）；仅 ADMIN 可空（全量）。所有查询/写操作强制附加归属条件（组合成 SQL `where AND`，避免泄漏）；不允许「空 = 全量」的放行逻辑（`scopeAllows` 系列已改为空 scope 即拒绝，ADMIN 例外，已实现 2026-09-01）
  - **零售（RETAILER）例外**：库存/零售价的查询范围 = **已签约仓库集合**（见 §3.2.1），**不**附加自身门店 `scope_unit_id` 过滤——当前 `GET /stock`、`GET /retail-prices` 以 `unitId || scope.unitId` 兜底会把零售限制到自己的门店（与「零售看各仓库库存」冲突），已修复（2026-09-01：按已签约仓库过滤，显式传未签约 unitId → 404）
...
| carrier / tracking_no | **配送商 + 配送单号**（2026-08-31 新增：仓库发送/发货时填写；零售只读查看，独立于发货单 `shipment_trackings`） |
...
#### 仓库-零售签约 `retail_partnerships`（2026-08-31 新增，已实现（2026-09-01））
`id, warehouse_unit_id, retailer_unit_id, created_by, created_at` · `UNIQUE(warehouse_unit_id, retailer_unit_id)`
- 由**仓库侧主动**添加（零售无需同意，无确认状态）；删除即解约。
- 零售的库存/零售价/可请货仓库范围 = 该零售门店已签约仓库集合。
...
10. **角色权限模型偏差（2026-08-31 发现，2026-08-31 晚语义已确认 → 已实现（2026-09-01））**：已实现能力与业务语义不符——RETAILER 应**不可见发货单**、**不可管理物品**、零售价**仅只读**（范围=已签约仓库）；仓库/集货/零售**必须绑定归属单元**（一个账户只能绑定一个实体，一个实体可有多个账户；仅 ADMIN 例外）；已新增（2026-09-01）**仓库-零售签约关系**（§3.2.1）与**销售单配送方式+单号**（§3.2.2）。详见 `docs/qa/rbac-matrix-vs-semantics.md`（§1.1 确认结果）与 `docs/qa/ui-acceptance.md` §8.1。
...
（2026-08-31 按业务语义修订：RETAILER = 外部合作方（商铺买家），不可见发货单、不管理零售价（仅只读，范围=已签约仓库）、不可管理物品；WAREHOUSE/COLLECTOR/RETAILER 必须绑定归属单元，一个账户只能绑定一个实体（实体可多账户），仅 ADMIN 例外。）

| 能力 | COLLECTOR | WAREHOUSE | RETAILER | ADMIN |
|---|---|---|---|---|
| 工作台/待办与通知 | ✓ | ✓ | ✓ | ✓ |
| 物品搜索/浏览/扫码 | ✓ | ✓ | ✓ | ✓ |
| 物品新增/编辑（全局目录） | ✓ | ✓ | ✗（仅内部角色维护） | ✓ |
| 发货单：创建/编辑/转交 | ✓ | 只读 | ✗ | ✓ |
| 发货单：绑定/查看多物流单号 | ✓ | ✓ | ✗（随发货单不可见） | ✓ |
| 点货（填实收）、提交差异修订 | 只读 | ✓ | ✗ | ✓ |
| 差异修订审批/拒绝 | ✓ | ✗ | ✗ | ✓ |
| 确认收货/入库建档（批次/效期） | ✗ | ✓ | ✗ | ✓ |
| 发货拒收 → 退货单；处理发货退货单 | ✓（处理） | ✓（发起） | ✗ | ✓ |
| 手动入库/出库/报损（含过期报损） | ✗ | ✓ | ✗ | ✓ |
| 零售价：查看（只读） | ✗ | ✓ | ✓（只读，范围=已签约仓库） | ✓ |
| 零售价：设置/修改（本仓库） | ✗ | ✓ | ✗ | ✓ |
| 库存/批次效期：查看 | ✗ | ✓（本仓库） | ✓（**已签约仓库**） | ✓（全部） |
| 请货（创建请货草稿，可选仓库=已签约） | ✗ | ✗ | ✓ | ✓ |
| 销售单：行级改价/折扣/发送/取消 | ✗ | ✓ | ✗ | ✓ |
| 销售单：查看配送方式+配送单号 | — | — | ✓（只读，仓库提供） | ✓ |
| 仓库-零售签约（可售客户列表维护） | ✗ | ✓（仅本仓库） | ✗（无需操作，自动生效） | ✓ |
| 上传支付凭证/确认收货 | ✗ | ✗ | ✓ | ✓ |
| 售后退货：发起 | ✗ | ✗ | ✓ | ✓ |
| 售后退货：审核/退收回补 | ✗ | ✓ | ✗ | ✓ |
| 用户/业务单元/审计/邮件管理 | ✗ | ✗ | ✗ | ✓ |

> 数据范围：ADMIN = 全部；WAREHOUSE = 归属仓库（只能管理自己归属的仓库）；COLLECTOR = 归属集货部/自己经手的发货单；RETAILER = 所属门店（买方），库存/零售价/可请货仓库按**已签约仓库**只读可见（签约由仓库主动添加，见 §3.2.1）。**「scope 空 = 全量」仅 ADMIN 有效**：非 ADMIN 的 `scope_unit_id` 必填（2026-08-31 确认，见 §3.3、§11.10）。
> ADMIN 执行时皆写入审计日志。
